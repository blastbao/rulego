...
type FunctionsRegistry struct {
	...
	functions map[string]func(ctx types.OperatorContext, msg types.RuleMsg)
	sync.RWMutex
}

//Register 注册函数
func (x *FunctionsRegistry) Register(functionName string, f func(ctx types.OperatorContext, msg types.RuleMsg)) {
	x.Lock()
	defer x.Unlock()
	if x.functions == nil {
		x.functions = make(map[string]func(ctx types.OperatorContext, msg types.RuleMsg))
	}
...
//Get 获取函数
func (x *FunctionsRegistry) Get(functionName string) (func(ctx types.OperatorContext, msg types.RuleMsg), bool) {
	x.RLock()
	defer x.RUnlock()
	if x.functions == nil {
		return nil, false
...
func (x *FunctionsNode) New() types.Operator {
	return &FunctionsNode{}
}

//Init 初始化
func (x *FunctionsNode) Init(configuration types.Config) error {
	return maps.Map2Struct(configuration, &x.Config)
}

//OnMsg 处理消息
func (x *FunctionsNode) OnMsg(ctx types.OperatorContext, msg types.RuleMsg) error {
