...
func BenchmarkChainNotChangeMetadata(b *testing.B) {
	...
	ruleEngine, err := rulego.NewEngine(str.RandomStr(10), []byte(ruleChainFile), rulego.WithConfig(config))
	if err != nil {
		b.Error(err)
	}
	for i := 0; i < b.N; i++ {
...
func BenchmarkCallRestApiNodeGo(b *testing.B) {
	//不使用协程池
	config := rulego.NewConfig()
	ruleEngine, _ := rulego.NewEngine(str.RandomStr(10), loadFile("./chain_call_rest_api.json"), rulego.WithConfig(config))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
//	}
//}
func callRestApiNode(ruleEngine *rulego.Engine) {
	metaData := types.NewMetadata()
	metaData.PutValue("productType", "test01")
